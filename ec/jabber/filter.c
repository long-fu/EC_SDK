...
iks_filter_remove_hook(iksfilter *f, iksFilterHook *filterHook)
{
	...
	{
		tmp = rule->next;
		if (rule->filterHook == filterHook)
			iks_filter_remove_rule(f, rule);
		rule = tmp;
	}
}
...
iks_filter_packet(iksfilter *f, ikspak *pak)
{
	...
	while (rule)
	{
		...
		{
			if (iks_strcmp(rule->id, pak->id) == 0)
				score += 16;
			else
				fail = 1;
		}
		...
		{
			if (iks_strcmp(rule->ns, pak->ns) == 0)
				score += 4;
			else
				fail = 1;
		}
		...
		{
			if (pak->from && iks_strcmp(rule->from, pak->from->full) == 0)
				score += 8;
			else
				fail = 1;
		}
		...
		{
			if (pak->from && iks_strcmp(rule->from, pak->from->partial) == 0)
				score += 8;
			else
				fail = 1;
		}
	...
	while (max_rule)
	{
		if (IKS_FILTER_EAT == max_rule->filterHook(max_rule->user_data, pak))
			return;
		max_rule->score = 0;
		max_rule = NULL;
		max_score = 0;
